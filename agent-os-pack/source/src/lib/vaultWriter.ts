...
// ─── IN-MEMORY LOCK ───────────────────────────────────────────────
// Simple mutex map for vault writes. Prevents write conflicts when both
// the Next.js dashboard and the Telegram bot write to the same daily file.
// For single-process Node.js this is sufficient and race-free.
const locks = new Map<string, Promise<void>>();

async function withLock<T>(key: string, fn: () => Promise<T>): Promise<T> {
  // Chain onto existing lock for this key
  const prev = locks.get(key) || Promise.resolve();
  let resolveLock: () => void;
  const thisLock = new Promise<void>((r) => { resolveLock = r; });
  locks.set(key, thisLock);

  await prev; // Wait for previous lock holder
  try {
    return await fn();
  } finally {
    resolveLock!();
    if (locks.get(key) === thisLock) locks.delete(key);
  }
}
...
export async function appendMemory(entry: {
  ...
  try {
    return await withLock(file, async () => {
      ...
      const sourceTag = entry.meta?.source ? ` · ${entry.meta.source}` : "";
      const block = [
        `## ${nowHM()} · ${entry.agent} · ${entry.kind}${sourceTag}`,
        entry.user ? `\n**You:** ${entry.user}\n` : "",
