...
// Asta Workspace — browse + preview everything the Codex agent has built.
...
export default function AstaWorkspace() {
        ...
        <div className="flex-1 min-h-0 overflow-y-auto scroll space-y-1">
          {projects.length === 0 && (
            <div className="px-2 py-3 text-[11px] text-[var(--cream-mute)] leading-snug">
              Nothing yet. Ask Asta to build something in the Chat tab — files land here.
            </div>
          )}
          {projects.map((p) => (
            <button key={p.name}
